...
-- modified by Marek Guminski
-- LICENSED UNDER GPL v2

-- File: cgen_ipbus_addr_table.lua
--
...
function cgen_ipbus_field_define(field, reg)
   local prefix;
   -- anonymous field?
...
function cgen_ipbus_ramdefs(ram)
...
function cgen_ipbus_field_masks()
   foreach_reg({TYPE_REG}, function(reg)
         ...
         if(reg.num_fields ~= nil and reg.num_fields > 0) then
            ...
            if ( reg.num_fields == 1) then 
               ...
               indent_right()
               foreach_subfield(reg, function(field, reg) cgen_ipbus_field_define(field, reg) end);
               indent_left()
               emit("</node>");
            end
         end
...
function cgen_ipbus_fileheader()
   emit ("<!--");
   emit ("  Register definitions for slave core: "..periph.name);
   emit ("");
   emit ("  * File           : "..options.output_ipbus_addr_file);
...
function cgen_generate_ipbus_addr_code()
   cgen_new_snippet();
   
   cgen_ipbus_fileheader();
   
   emit( "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>\n<node>)" )
   
   indent_right();
   cgen_ipbus_field_masks();
   indent_left();
